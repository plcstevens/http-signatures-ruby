  ...
  context 'with one key in KeyStore, no signing_key_id specified' do
    ...
    describe '#signer' do
      it 'instantiates Signer with key, algorithm, headers' do
        allow(HttpSignatures::Signer).to receive(:new)

        context.signer

        expect(HttpSignatures::Signer).to have_received(:new) do |args|
      ...
      it 'signs without errors' do
        expect { context.signer.sign(message) }.not_to raise_error
      end
      ...
      it 'verifies without errors' do
        ...
        expect { context.verifier.valid?(message) }.not_to raise_error
      end
    end
  end
