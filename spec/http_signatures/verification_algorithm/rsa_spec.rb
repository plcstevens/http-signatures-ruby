...
RSpec.describe HttpSignatures::VerificationAlgorithm::Rsa do
  ...
  context 'when the algorithm is rsa-sha1' do
    let(:algorithm) { 'rsa-sha1' }

    context 'when headers are %w{date}' do
      let(:headers) { %w[date] }

      it_behaves_like 'signer',
                      'kcX/cWMRQEjUPfF6AO7ANZ/eQkpRd' \
    ...
    context 'when headers are %w{(request-target) host date}' do
      let(:headers) { %w[(request-target) host date] }

      it_behaves_like 'signer',
                      'F6g4qdBSHBcWo1iMsHetQU9TnPF39' \
  ...
  context 'when the algorithm is sha384' do
  ...
  context 'when the algorithm is sha512' do
